...
call robocopy "%~dp0ThirdParty/OGRE/Bin/debug" "%~dp0bin/debug" *.dll *.cfg /np /r:1 /xo
if %errorlevel% GEQ 8 exit /b %errorlevel%
call robocopy "%~dp0ThirdParty/SDL2/Bin" "%~dp0bin/debug" SDL2_d.dll /np /r:1 /xo
if %errorlevel% GEQ 8 exit /b %errorlevel%
call robocopy "%~dp0ThirdParty/OGRE/Bin/release" "%~dp0bin/release" *.dll *.cfg /np /r:1 /xo
if %errorlevel% GEQ 8 exit /b %errorlevel%
call robocopy "%~dp0ThirdParty/SDL2/Bin" "%~dp0bin/release" SDL2.dll /np /r:1 /xo
if %errorlevel% GEQ 8 exit /b %errorlevel%
call robocopy "%~dp0ThirdParty/OGRE/Bin/release" "%~dp0bin/shipping" *.dll *.cfg /np /r:1 /xo
if %errorlevel% GEQ 8 exit /b %errorlevel%
